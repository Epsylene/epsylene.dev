...
export async function Card({ title, link, image, children }: ProjectCardProps) {
  ...
  return (
    <div className="break-inside-avoid mb-6 group">
      ...
      <div
        className="p-6 pt-4 pb-3 rounded-b-md transition-all group-hover:brightness-110 [&_a:hover]:opacity-80"
        style={{ backgroundColor: `${color}` }}
      >
        {
          link ? (
            <Link href={link} target="_blank" rel="noopener noreferrer">
              <div className="flex items-center text-2xl font-bold gap-2 pb-4">
